...
1. Run `ansur whoami`. If it errors with `no_tenant` (or `ansur` isn't installed /
   ...
   first, then continue.
2. Otherwise walk the build loop in the skill (`SKILL.md`): connect the systems
   the role needs → `bundle create` → author the bundle → `git push` → wire a
   channel → `trace` and iterate.
